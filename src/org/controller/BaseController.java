...
public class BaseController {
	...
	public static ArrayList<Student> getList(){
		...
		String sql="select * from student join t ";
	...
	public static int update(Student one){
		int result=0;
		String sql = "update student set sname='%s',sage='%s',sbirth='%s',saddress='%s' where sno=%s";
		DBBean jdbc=new DBBean();
		result=jdbc.executeUpdate(String.format(sql, one.getSname(),one.getSage(),one.getSbirth(),one.getSaddress(),one.getSno()));
		jdbc.close();
		return result;
	}
	...
	public static int add(Student one){
		int result=0;
		String sql = "insert into student(sno,sname,sage,sbirth,saddress) values('%s','%s',%d,'%s','%s')";
		DBBean jdbc=new DBBean();
		
		result=jdbc.executeUpdate(String.format(sql, one.getSno(),one.getSname(),one.getSage(),one.getSbirth(),one.getSaddress()));
		jdbc.close();
		return result;
	}
}
